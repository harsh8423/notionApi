...
function Page(props) {
  ...
  const handlePage=(id)=>{
    ...
    navigate("../page2", { state: id, paged:true });
  }
      ...
      {NotionData?.results?.map((block) => {
        ...
        switch (block.type) {
          ...
          case 'callout':
            ...
            return <Paragraph key={id} callout={block.callout.color} text={callArray} id={id}/>;
          case 'child_page':
            return <div className='para' style={{textDecoration:"underline", fontWeight:"bold", cursor:"pointer", paddingLeft:"13px"}} onClick={()=>handlePage(id)}>Page: {block.child_page.title}</div>
